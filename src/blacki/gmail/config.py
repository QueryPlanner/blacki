...
def resolve_gmail_redirect_uri(
    ...
    """Resolve the Gmail callback, deriving its host from Google Health when set."""
    ...
    health_uri = values.get("GOOGLE_HEALTH_REDIRECT_URI", "").strip()
    health_path = "/integrations/google-health/callback"
    if health_uri and health_path in health_uri:
        derived = health_uri.replace(health_path, "/integrations/gmail/callback", 1)
        _validate_redirect_uri(derived)
        return derived
...
class GmailConfig:
    ...
    def from_environment(
        ...
        """Build Gmail settings whenever shared Google values are present."""
        values = environ if environ is not None else os.environ
