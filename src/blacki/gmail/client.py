...
class GmailCredentialManager:
    ...
    async def get_access_token(self, telegram_user_id: str) -> str:
        ...
        if cached is not None and now < cached.expires_at - TOKEN_EXPIRY_GRACE_SECONDS:
            return cached.value
        ...
        async with user_lock:
            ...
            if isinstance(rotated_refresh_token, str) and rotated_refresh_token:
                ...
                await self.storage.replace_refresh_token(user_id, encrypted)
...
class GmailApiClient:
    ...
    async def _request(
        ...
        *,
        ...
        raise GmailTransportError(  # pragma: no cover - range(2) always returns/raises
            "Gmail request did not complete"
        )
    ...
    async def get_full_message(
        ...
        *,
        ...
        _reject_restricted_mailbox(payload)
        return payload
    ...
    async def get_thread(
        ...
        *,
        ...
        for raw_message in raw_messages:
            _reject_restricted_mailbox(_object(raw_message, "Gmail thread message"))
        messages = [
            _normalize_message(_object(item, "Gmail thread message"))
            for item in raw_messages[:count]
        ]
        return {
    ...
    async def modify_labels(
        ...
        *,
        ...
        await self._ensure_modifiable_mailbox_resource(
            telegram_user_id,
            resource=resource,
    ...
    async def _ensure_modifiable_mailbox_resource(
        self,
        telegram_user_id: str,
        *,
        resource: str,
        ...
        """Reject label changes for messages or threads in spam or trash."""
        payload = await self._request(
            telegram_user_id,
            "GET",
        ...
        if resource == "messages":
            _reject_restricted_mailbox(payload)
            return
        messages = _object_list(payload.get("messages"), "messages", "Gmail thread")
...
class GmailService:
    ...
    async def disconnect(self, user_id: str) -> bool:
        """Revoke remotely before removing only this user's local row."""
        user_id = _require_user_id(user_id)
        connection = await self.storage.get_connection(user_id)
        if connection is None:
            return False
        if connection.encrypted_refresh_token:
            try:
                ...
                await self.storage.mark_revocation_required(user_id)
                self.credentials.invalidate(user_id)
                raise GmailRevocationError(
                    "Stored Gmail credential could not be revoked; retry disconnect"
                ) from None
            else:
                revoked = await self.api.revoke(refresh_token)
                if not revoked:
                    await self.storage.mark_revocation_required(user_id)
                    self.credentials.invalidate(user_id)
                    raise GmailRevocationError(
...
def _draft_matches(
    ...
    *,
    ...
    try:
        ...
        _validate_content_fingerprint(expected_content_fingerprint)
    ...
    return (
        ...
        and message.get("content_fingerprint") == expected_content_fingerprint
    )


def _message_content_fingerprint(message: Mapping[str, Any]) -> str:
    """Fingerprint draft content without returning its content in confirmations."""
    content = {
...
def _validate_content_fingerprint(value: str) -> None:
    if (
        not isinstance(value, str)
        or len(value) != 64
        or any(char not in "0123456789abcdef" for char in value)
    ):
        raise GmailInputError("Gmail draft content fingerprint is invalid")
...
def _normalize_message(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    def visit(part: Mapping[str, Any]) -> None:
        ...
        if raw_parts is not None:
            for child in _object_list(raw_parts, "parts", "Gmail message parts"):
                visit(_object(child, "Gmail message part"))
    ...
    normalized["content_fingerprint"] = _message_content_fingerprint(normalized)
    return normalized
...
def _find_message_part(
    ...
    def visit(part: Mapping[str, Any]) -> Mapping[str, Any] | None:
        ...
        if raw_parts is None:
            return None
        for child in _object_list(raw_parts, "parts", "Gmail message parts"):
            found = visit(_object(child, "Gmail message part"))
...
def _reject_restricted_mailbox(payload: Mapping[str, Any]) -> None:
    """Keep direct-ID reads and label changes out of spam and trash."""
    label_ids = _string_list(payload.get("labelIds", []), "labelIds")
    if {label.upper() for label in label_ids}.intersection({"SPAM", "TRASH"}):
        raise GmailInputError("Gmail access to spam and trash is disabled")
