...
def _truncate_gmail_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    compact = cast(dict[str, Any], _truncate_gmail_bodies(result))
...
def _truncate_gmail_bodies(value: object) -> dict[str, Any] | list[Any] | object:
    if isinstance(value, dict):
        truncated: dict[str, Any] = {}
        for key, item in value.items():
            if key in {"text_body", "html_body"} and isinstance(item, str):
                truncated[key] = item[:GMAIL_INLINE_BODY_CHAR_LIMIT]
            else:
                truncated[key] = _truncate_gmail_bodies(item)
        return truncated
    if isinstance(value, list):
        return [_truncate_gmail_bodies(item) for item in value]
    return value
...
async def gmail_create_draft(
    ...
    *,
    ...
    async with _service_for_context(tool_context) as (service, user_id):
        ...
        return await _materialize_large_gmail_result(
            result,
            identity=f"draft-create:{to}:{subject}",
            tool_context=tool_context,
        )
...
async def gmail_send_draft(
    ...
    *,
    ...
    """Send a confirmed draft only if its metadata and content still match."""
    async with _service_for_context(tool_context) as (service, user_id):
        ...
        return await _materialize_large_gmail_result(
            result,
            identity=f"draft-send:{draft_id}",
            tool_context=tool_context,
        )
