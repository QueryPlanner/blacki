...
- **Drafting & Replies**:
  ...
  - Use `gmail_send_draft` only after Google ADK displays the exact draft ID, recipients, and subject and the user explicitly confirms. The tool reloads the draft immediately before sending and stops if those values changed.
- **Labels & Organization**:
