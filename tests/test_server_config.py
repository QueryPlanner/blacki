...
async def test_gmail_callback_validates_state_and_errors(
    ...
    import blacki.server as server
    from blacki.gmail import GmailApiError, GmailOAuthError
...
@pytest.mark.asyncio
async def test_gmail_start_and_stop_are_optional(
    mock_dependencies: MagicMock,
) -> None:
    if "blacki.server" in sys.modules:
        del sys.modules["blacki.server"]
    ...
    import blacki.server as server
    from blacki.gmail import GmailConfig

    server._container = None
    await server._start_gmail()

    server._container = MagicMock()
    with patch("blacki.gmail.GmailConfig.from_environment", return_value=None):
        await server._start_gmail()
    ...
    with patch(
        ...
        side_effect=__import__(
            "blacki.gmail", fromlist=["GmailConfigurationError"]
        ).GmailConfigurationError("bad config"),
    ):
        await server._start_gmail()
    assert server._gmail_oauth_service is None
    ...
    with (
        ...
        patch("blacki.gmail.GmailOAuthService", return_value=fake_service),
    ):
        await server._start_gmail()
    ...
    failing_service.close = AsyncMock(side_effect=RuntimeError("close failure"))
