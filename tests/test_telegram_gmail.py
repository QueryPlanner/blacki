...
@pytest.mark.asyncio
async def test_connect_gmail_handles_credential_failure_without_details() -> None:
    bot, service, api = _bot()
    service.begin_authorization.side_effect = GmailCredentialError("secret details")
...
async def test_disconnect_gmail_clears_temporary_sandbox_artifacts(
    cleanup_fails: bool,
) -> None:
    bot, service, api = _bot()
    session = MagicMock(state={"__sandbox_id__": "sandbox-1"})
    manager = MagicMock()
    manager.clear_gmail_artifacts = AsyncMock(
        side_effect=RuntimeError("cleanup details") if cleanup_fails else None
    )
    message = _message(command="/disconnect_gmail")
    ...
    manager.clear_gmail_artifacts.assert_awaited_once()
    assert "Gmail was disconnected" in api.send_message.call_args.kwargs["text"]


@pytest.mark.asyncio
async def test_disconnect_gmail_reports_already_disconnected() -> None:
    bot, service, api = _bot()
    ...
    assert "already disconnected" in api.send_message.call_args.kwargs["text"]
