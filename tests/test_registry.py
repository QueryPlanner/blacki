...
class TestBuildTools:
    ...
    def test_browser_takeover_is_private_root_only(self) -> None:
        """Browser takeover must never reach public or worker agents."""
        config = ToolConfig(
            weather_enabled=False,
            sandbox_enabled=True,
...
class TestBuildToolConfigFromEnv:
    ...
    def test_invalid_gmail_settings_from_env_disable_only_gmail(self) -> None:
        ...
        with patch.dict(
            "os.environ",
            {
                ...
                "GOOGLE_HEALTH_TOKEN_ENCRYPTION_KEY": "invalid",
            },
            clear=False,
        ):
            config = build_tool_config_from_env()
    ...
    def test_gmail_fallback_settings_from_env(self) -> None:
        """Gmail should reuse the Google Health OAuth settings."""
        from cryptography.fernet import Fernet
        ...
        with patch.dict(
            "os.environ",
            {
                ...
                "GOOGLE_HEALTH_CLIENT_SECRET": "health-client-secret",
        ...
        assert config.gmail_config is not None
        assert config.gmail_config.client_id == "health-client-id"
