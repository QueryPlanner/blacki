    ...
    config = GmailConfig.from_environment(
        {
            ...
            "GMAIL_REDIRECT_URI": "https://example.test/integrations/gmail/callback",
        }
    )
...
def test_gmail_config_rejects_missing_and_insecure_values() -> None:
    ...
    with pytest.raises(GmailConfigurationError, match="incomplete"):
        GmailConfig.from_environment(
            {"GMAIL_ENABLED": "true", "GMAIL_CLIENT_ID": "client"}
        )
...
def test_gmail_config_accepts_explicit_and_local_redirects() -> None:
    ...
    configured_limit = GmailConfig.from_environment(
        {
            "GMAIL_ENABLED": "true",
            "GMAIL_CLIENT_ID": "client",
            "GMAIL_CLIENT_SECRET": "secret",
            "GMAIL_TOKEN_ENCRYPTION_KEY": Fernet.generate_key().decode(),
            "GMAIL_MAX_ATTACHMENT_BYTES": "5",
        }
    )
    assert configured_limit is not None
    assert configured_limit.max_attachment_bytes == 5


def test_gmail_config_rejects_invalid_attachment_limits() -> None:
    key = Fernet.generate_key().decode()
    with pytest.raises(GmailConfigurationError):
        GmailConfig(
            client_id="client",
            ...
            GmailConfig.from_environment(
                {
                    ...
                    "GMAIL_CLIENT_SECRET": "secret",
                    "GMAIL_TOKEN_ENCRYPTION_KEY": key,
                    "GMAIL_MAX_ATTACHMENT_BYTES": value,
                }
            )
...
async def test_gmail_storage_encrypts_and_isolates_connections(tmp_path: Path) -> None:
    ...
    try:
        ...
        await storage.save_connection(
            telegram_user_id="telegram-chat-42",
            encrypted_refresh_token=config.cipher.encrypt("refresh-pending"),
            scopes=(GMAIL_SCOPE,),
        )
        await storage.mark_revocation_required("telegram-chat-42")
        pending = await storage.get_connection("telegram-chat-42")
        assert pending is not None
        assert pending.status == "revocation_required"
        assert pending.encrypted_refresh_token
        assert await storage.has_connection("telegram-chat-42") is False
        with pytest.raises(GmailAlreadyConnectedError):
            await storage.save_connection(
                telegram_user_id="telegram-chat-42",
