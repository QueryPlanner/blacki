...
@pytest.mark.asyncio
async def test_direct_reads_and_label_changes_reject_spam_and_trash(
    tmp_path: Path,
) -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        if request.url == httpx.URL("https://oauth2.googleapis.com/token"):
        ...
        if request.url.path.endswith("/threads/thread-1"):
            return httpx.Response(
                ...
                json={
                    "id": "thread-1",
                    "messages": [{**_message(), "labelIds": ["SPAM"]}],
                },
                request=request,
            )
        if request.url.path.endswith("/messages/message-1"):
    ...
    client, _, service, connection = await _ready(tmp_path, handler)
    try:
        with pytest.raises(GmailInputError, match="spam and trash"):
            await service.get_message(USER_ID, message_id="message-1")
        with pytest.raises(GmailInputError, match="spam and trash"):
            await service.get_thread(USER_ID, thread_id="thread-1")
        with pytest.raises(GmailInputError, match="spam and trash"):
...
@pytest.mark.parametrize(
    ("value", "error"),
    [
        (None, GmailMalformedResponseError),
        ("", GmailMalformedResponseError),
        ("!", GmailMalformedResponseError),
        ("a" * 20, GmailInputError),
        ("YWJj", GmailInputError),
    ],
)
def test_attachment_data_decoder_rejects_invalid_and_oversized_values(
    value: object,
    error: type[Exception],
) -> None:
    with pytest.raises(error):
        _decode_attachment_data(value, max_bytes=2)
    ...
    if value == "YWJj":
        with (
            ...
            _decode_attachment_data("AA==", max_bytes=2)


def test_message_part_lookup_walks_nested_parts_and_handles_missing_parts() -> None:
    payload = {
        "payload": {
            "parts": [
                {
    ...
    assert _find_message_part(payload, "target") == {
        "partId": "target",
        "body": {},
    }
    assert _find_message_part({"payload": {"parts": None}}, "target") is None
    assert _find_message_part({"payload": {"parts": []}}, "target") is None
...
@pytest.mark.asyncio
async def test_attachment_download_validates_parts_sizes_and_sources(
    tmp_path: Path,
) -> None:
    mode = "external"
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path.endswith("/messages/message-1"):
            ...
            if mode == "external":
                ...
                attachment_body = {"data": _encoded("data"), "size": 4}
            elif mode == "no-size":
                attachment_body = {"data": _encoded("data")}
            elif mode == "negative":
                attachment_body = {"data": _encoded("data"), "size": -1}
            elif mode == "oversized":
                attachment_body = {"data": _encoded("data"), "size": 6}
            else:
                attachment_body = {"data": _encoded("data"), "size": 5}
            return httpx.Response(
                200,
                json=_message(
                    extra_parts=[
    ...
    client, _, service, connection = await _ready(
        tmp_path,
        handler,
        config=replace(_config(), max_attachment_bytes=5),
    )
    try:
        external = await service.download_attachment(
            USER_ID,
            message_id="message-1",
            part_id="1",
        )
        assert external.filename == "attachment"
        assert external.mime_type == "application/octet-stream"
        assert external.data == b"data"

        mode = "inline"
        inline = await service.download_attachment(
            USER_ID,
            message_id="message-1",
            part_id="1",
        )
        assert inline.data == b"data"

        mode = "no-size"
        no_size = await service.download_attachment(
            USER_ID,
            message_id="message-1",
            part_id="1",
        )
        assert no_size.data == b"data"

        for invalid_mode, error in (
            ("missing-id", GmailMalformedResponseError),
            ("missing-thread", GmailMalformedResponseError),
            ("missing-payload", GmailMalformedResponseError),
            ("missing-part", GmailInputError),
            ("negative", GmailMalformedResponseError),
            ("oversized", GmailInputError),
            ("mismatch", GmailMalformedResponseError),
        ):
            mode = invalid_mode
            with pytest.raises(error):
                await service.download_attachment(
...
async def test_draft_creation_reply_and_send_recheck(tmp_path: Path) -> None:
    ...
    try:
        ...
        result = await service.send_draft(
            ...
            expected_content_fingerprint=draft["message"]["content_fingerprint"],
        )
...
async def test_draft_creation_includes_optional_recipients_without_message_id(
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path.endswith("/drafts"):
            ...
            raw = body["message"]["raw"]
            decoded = base64.urlsafe_b64decode(raw + "=" * (-len(raw) % 4)).decode()
            if "Cc: copy@example.com" in decoded:
    ...
    try:
        ...
        standalone = await service.create_draft(
            USER_ID,
            to="person@example.com",
            subject="Standalone",
            body="body",
        )
        assert standalone["draft_id"] == "draft-1"
    finally:
        await _close(client, service, connection)
...
async def test_labels_and_input_bounds(tmp_path: Path) -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path.endswith("/messages/message-1"):
            return httpx.Response(
                200,
                json={"id": "message-1", "labelIds": ["INBOX"]},
                request=request,
            )
...
def test_draft_match_requires_send_metadata() -> None:
    ...
    for missing in ("to", "subject"):
        incomplete: dict[str, Any] = {"headers": dict(message["headers"])}
    ...
    for invalid_fingerprint in ("", "A" * 64, "x" * 63):
        assert (
            _draft_matches(
                message,
                expected_to="person@example.com",
                ...
                expected_bcc="",
                expected_subject="Subject",
                expected_content_fingerprint=invalid_fingerprint,
            )
            is False
        )
...
@pytest.mark.asyncio
async def test_gmail_api_validates_attachment_resource_ids(tmp_path: Path) -> None:
    async def handler(request: httpx.Request) -> httpx.Response:
    ...
    try:
        with pytest.raises(GmailInputError):
            await service.api.get_attachment(
                USER_ID,
                message_id="bad/id",
                attachment_id="attachment-1",
            )
