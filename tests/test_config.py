...
from blacki.utils.config import (
    ServerEnv,
...
class TestServerEnv:
    ...
    def test_telegram_configuration_requires_enabled_token(
        self, valid_server_env: dict[str, str]
    ) -> None:
        """Telegram requires an enabled configuration and a token."""
        valid_server_env["TELEGRAM_ENABLED"] = "true"
        valid_server_env["TELEGRAM_BOT_TOKEN"] = "test-token-123"  # noqa: S105
        env = ServerEnv.model_validate(valid_server_env)
        assert env.is_telegram_configured is True
    ...
    def test_session_uri_property(self, valid_server_env: dict[str, str]) -> None:
        ...
        # Case 2: Only agent_engine -> uses agent_engine_uri
        data = {**valid_server_env, "AGENT_ENGINE": "test-engine-id"}
        env = ServerEnv.model_validate(data)
