...
@pytest.mark.asyncio
async def test_notify_after_model_sends_tool_only_notification(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Send tool notification when model makes tool call without text.

    This handles the case where some models (Grok, DeepSeek) may only return
    a function call without accompanying text.
    """
    monkeypatch.setenv("TELEGRAM_ENABLED", "true")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "tok")
    monkeypatch.setenv("TELEGRAM_TOOL_NOTIFICATIONS", "true")
    ...
    # Tool name is escaped for MarkdownV2, so underscore becomes \_
    assert "test\\_tool" in kwargs["text"]


@pytest.mark.asyncio
async def test_notify_after_model_sends_multi_tool_notification(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Send notification with multiple tool names when model calls multiple tools."""
    monkeypatch.setenv("TELEGRAM_ENABLED", "true")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "tok")
    monkeypatch.setenv("TELEGRAM_TOOL_NOTIFICATIONS", "true")
    ...
    with patch("blacki.callbacks.TelegramApiClient", return_value=mock_client):
        ...
        tool_call_part1 = Part(
            function_call=FunctionCall(name="search_memory", args={})
        )
        tool_call_part2 = Part(function_call=FunctionCall(name="get_weather", args={}))

        response = LlmResponse(
            content=Content(parts=[tool_call_part1, tool_call_part2])
        )
        await callbacks_module.notify_telegram_after_model(ctx, response)

    mock_client.send_message.assert_awaited_once()
    kwargs = mock_client.send_message.await_args.kwargs
    assert "Using tools" in kwargs["text"]
    assert "search\\_memory" in kwargs["text"]
    assert "get\\_weather" in kwargs["text"]
...
# Tests for _send_tool_only_notification edge cases
@pytest.mark.asyncio
async def test_send_tool_only_notification_missing_chat_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Skip tool-only notification when chat_id is missing."""
    monkeypatch.setenv("TELEGRAM_ENABLED", "true")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "tok")
    monkeypatch.setenv("TELEGRAM_TOOL_NOTIFICATIONS", "true")
    ...
    with patch("blacki.callbacks.TelegramApiClient", return_value=mock_client):
        ctx = MagicMock(spec=CallbackContext)
        ctx.state = MockState({})  # No telegram_chat_id

        await callbacks_module._send_tool_only_notification(ctx, ["test_tool"])

    mock_client.send_message.assert_not_called()


@pytest.mark.asyncio
async def test_send_tool_only_notification_invalid_chat_id(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Skip tool-only notification when chat_id is invalid."""
    monkeypatch.setenv("TELEGRAM_ENABLED", "true")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "tok")
    monkeypatch.setenv("TELEGRAM_TOOL_NOTIFICATIONS", "true")
...
@pytest.mark.asyncio
async def test_send_tool_only_notification_rate_limited(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Skip tool-only notification when rate limited."""
    monkeypatch.setenv("TELEGRAM_ENABLED", "true")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "tok")
    monkeypatch.setenv("TELEGRAM_TOOL_NOTIFICATIONS", "true")
    ...
    with patch("blacki.callbacks.TelegramApiClient", return_value=mock_client):
        ...
        await callbacks_module._send_tool_only_notification(ctx, ["tool1"])
        # Second call immediately should be rate limited
        await callbacks_module._send_tool_only_notification(ctx, ["tool2"])

    # Only one message should be sent (the first one)
    assert mock_client.send_message.await_count == 1


@pytest.mark.asyncio
async def test_send_tool_only_notification_api_error(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """Handle TelegramApiError in tool-only notification."""
    monkeypatch.setenv("TELEGRAM_ENABLED", "true")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "tok")
    monkeypatch.setenv("TELEGRAM_TOOL_NOTIFICATIONS", "true")
    ...
    assert "Telegram tool-only notification failed" in caplog.text


@pytest.mark.asyncio
async def test_send_tool_only_notification_unexpected_error(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
) -> None:
    """Handle unexpected error in tool-only notification."""
    monkeypatch.setenv("TELEGRAM_ENABLED", "true")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "tok")
    monkeypatch.setenv("TELEGRAM_TOOL_NOTIFICATIONS", "true")
    ...
    assert "Unexpected error sending Telegram tool-only notification" in caplog.text


@pytest.mark.asyncio
async def test_send_tool_only_notification_missing_token(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Skip tool-only notification when token is missing."""
    monkeypatch.setenv("TELEGRAM_ENABLED", "true")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "")
    monkeypatch.setenv("TELEGRAM_TOOL_NOTIFICATIONS", "true")
...
@pytest.mark.asyncio
async def test_notify_after_model_function_call_without_name(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Skip notification when function_call exists but has no name and no text."""
    monkeypatch.setenv("TELEGRAM_ENABLED", "true")
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "tok")
    monkeypatch.setenv("TELEGRAM_TOOL_NOTIFICATIONS", "true")
    ...
    with patch("blacki.callbacks.TelegramApiClient", return_value=mock_client):
        ...
        # Create a function call with no name (edge case)
        fc = FunctionCall(name="", args={})
        tool_call_part = Part(function_call=fc)

        response = LlmResponse(content=Content(parts=[tool_call_part]))
        await callbacks_module.notify_telegram_after_model(ctx, response)

    # Should not send any notification since tool name is empty and no text
    mock_client.send_message.assert_not_called()
