...
4. The link opens a live canvas. Mouse, touch, and keyboard events pass through
   Blacki to the Agent Browser stream without entering ADK session history.
5. Clicking **Done** disables the stream and resumes the waiting tool call.

The URL fragment is exchanged once for an HttpOnly, SameSite cookie. Blacki
stores only token hashes, sandbox routing details, and completion state in
process memory. Restarting Blacki invalidates every takeover.
